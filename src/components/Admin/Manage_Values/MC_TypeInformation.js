...
const MC_TypeInformation = () => {
  ...
  const onFinishEdit = async (values, id) => {
    try {
      ...
      const response = await fetch(
        `https://checkkonproject-sub.com/api/TypeInformation_update/${id}`,
        {
          method: "POST",
          body: formData,
        }
  ...
  const editRow = (record) => {
    form.setFieldsValue({
      type_info_name: record.type_info_name,
    });
  ...
  const handleDelete = async (id) => {
    try {
      const filteredItems = fakeNewsInfo.filter(
        (item) => item.mfi_ty_info === id
      );
      if (filteredItems.length > 0) {
        message.error("ไม่สามารถลบข้อมูลได้ เนื่องจากมีการใช้ข้อมูลนี้อยู่");
      } else {
        const response = await fetch(
          `https://checkkonproject-sub.com/api/TypeInformation_delete/${id}`,
          {
            method: "DELETE",
          }
        );
...
export default MC_TypeInformation;
