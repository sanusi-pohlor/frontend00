...
const MC_CheckingData = () => {
  ...
  const onFinishEdit = async (values, id) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://checkkonproject-sub.com/api/CheckingData_update/${id}`,
        {
          method: "POST",
          body: formData,
        }
  ...
  const editRow = (record) => {
    form.setFieldsValue({
      che_d_format: record.che_d_format,
    });
  ...
  const handleDelete = async (id) => {
    try {
      const filteredItems = fakeNewsInfo.filter(
        (item) => item.mfi_che_d === id
      );
      if (filteredItems.length > 0) {
        message.error("ไม่สามารถลบข้อมูลได้ เนื่องจากมีการใช้ข้อมูลนี้อยู่");
      } else {
        const response = await fetch(
          `https://checkkonproject-sub.com/api/CheckingData_delete/${id}`,
          {
            method: "DELETE",
          }
        );
        ...
        if (
          response.ok &&
          responseData === "CheckingData deleted successfully"
        ) {
          console.log("CheckingData deleted successfully");
          fetchData();
        } else {
