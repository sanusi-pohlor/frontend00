              ...
              filteredNews
                ...
                .map((item) => (
                  <Grid item xs={12} key={item.id} className="gridItem">
                    <Link to={`/News_Menu/News_view/${item.id}`} style={{ textDecoration: "none" }}>
                      <Card hoverable className="cardItem-head">
                        <div className="cardItemCover">
                          <Flex justify="space-between">
                            ...
                            <Flex vertical align="flex-end" justify="space-between" className="flex-card">
                              <strong>เผยแพร่ {moment(item.created_at).format('DD-MM-YYYY')}</strong>
                              <br />
                              {item.title}
                              <Button type="primary" href={`/News_Menu/News_view/${item.id}`} target="_blank" className="button-card">
                                อ่านต่อ
                              </Button>
                            </Flex>
                          </Flex>
